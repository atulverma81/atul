<?php echo $form->create('Event',array('action'=>'editEvent/','id'=>'formID','method'=>'POST','enctype'=>'multipart/form-data','class'=>'formular','name'=>'frmCreateEvent') )?>
 <!--Search Result Starts Here-->
	...
	<li>   <font class="requiredField">*</font> marked are mandatory</li>
	 ...
	 <?php echo $form->input('Event.event_keyword',array('id'=>'keyword','class'=>'validate[required]','type'=>'text','label'=>'Event Keyword<font class="requiredField">*</font>','div'=>false,'errors'=>true)) ?>
		<label></label><p class="color-red">Multiple words, separated by commas ( e.g. love, peace, house )</p></li> 
          </li>
	<li>
	 ...
	 <li id="showOtherFrequency" style="display:none"> 
	  ...
	  <li>
           ...
           <?php echo $form->input('Event.preimage', array('type'=>'hidden','value'=>$this->data['Event']['image'])); ?>
          </li> 
            <li>
            <label></label> 
	    <?php if(!empty($this->data['Event']['image'])){ ?>
              <img src="/img/eventImages/thumbs/<?php echo $this->data['Event']['image']; ?>" alt=""> 
	    <?php } else {?>
	...
	<li> 
	 ...
	 <li>
		...
		<?php
			...
			echo $form->select('User.country_id',$countries,null,array('id'=>'country','class'=>'validate[required]','default' => $this->data['Event']['country']),'Please select');
			}
	 ...
	 <li>
		...
		<?php 
			...
			e('<div id="StateDiv">');
			echo $form->select('User.state_id',array($states),$this->data['Event']['state'],array('id'=>'state','class'=>'validate[required]','onChange'=>'return showOtherState()'),'Please select');
			e('</div>');			 	
			}
		?>
          </li>
          <?php 
			if(!empty($this->data['Event']['otherstate']) && $this->data['Event']['state'] == 1110) {	 ?>
          
	  <li id="showOtherStateBox" >
	      <label>State Name<font class="requiredField">*</font></label>
		<?php echo $form->input('Event.otherstate',array('value'=>$this->data['Event']['otherstate'],'id'=>'statename','type'=>'text','label'=>false,'div'=>false,'errors'=>true)) ?>  
	 </li>	
	 <?php } ?>
