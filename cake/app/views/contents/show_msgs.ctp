...
<!--  right-content div starts here -->
		<div class="right-content">
			...
			<div class="travel-offers"style="min-height:200px;">
			<!--	<h4>Travel Offers</h4>
				<ul>
    ...
    <!-- Start: Content -->
	<?php echo $form->create('Content',array('action'=>'/update','name'=>'showMsgsForm','onSubmit'=>''))?>
	<div>
		<?php 			
				$options = array("class"=>"dropdown",
								'onChange'=>"document.showMsgsForm.submit()",
								'url'=>'/contents/update');							
				echo $form->select('Message.action',array('read'=>'Mark as Read','unread'=>'Mark as Unread','delete'=>'Delete'),@$this->data['Message']['action'],$options, '--- Select ---');
				//echo $ajax->observeField('MessageAction',$options);
			?>
	<br class="clear" />
    </div>
	<table width="100%" cellpadding="0" cellspacing="2" border="0" id="MsgList">
    ...
    $pagination->result();
        echo '<tr bgcolor=#f3f3f3 class="lft_head">
                ...
                <?php echo '<td width=20%>'.$pagination->sortBy('username','Sender','User',array('class'=>'lft_link','escape'=>false),'&nbsp;'.$html->image('paging_up.png',array('border'=>'0')),'&nbsp;'.$html->image('paging_down.png',array('border'=>'0'))).'</td>
    ...
    //Start: Row
        foreach ($data as $output)
        {
            ...
            echo '<td width=20%>'.$html->link(ucwords($output['User']['username']), "/contents/viewMsg/{$output['Message']['id']}",array('class'=>$class)).'</td>';	
    ...
    <?php echo $this->element('pagination'); // Render the pagination element
     ...
     <font color="green" size="3px">Empty Inbox..</font>
    <?php } ?>
    </td>
    </tr>
    </table>
